...
** 初期コンテナ作成
#+begin_src sh
docker exec -it walter_swift_xx \
...
** mongo
#+begin_src sh
docker exec -it mongo_cfg_xxx mongo --port 27017
docker exec -it mongo_srd_xxx mongo --port 27017
docker exec -it mongos_xx mongo --port 27017
#+end_src

** swift
#+begin_src sh
docker exec -it walter_swift_xx swift -A http://127.0.0.1:8080/auth/v1.0 -U test:tester -K testing stat walter
#+end_src
